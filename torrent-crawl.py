...
from apolloapi import ApolloAPI
...
def main():
    ...
    username = config.get('apollo', 'username')
    password = config.get('apollo', 'password')
    torrent_dir = os.path.expanduser(config.get('apollo', 'torrent_dir'))

    print 'Logging in to %s...' % __site_url__
    api = ApolloAPI(username, password)
